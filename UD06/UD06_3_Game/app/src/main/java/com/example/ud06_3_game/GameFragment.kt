...
//Inicializamos la palabra
        ...
        binding.btnNext.setOnClickListener {
            if (binding.textSecretWord.text.length > 0) {
                ...
                if (model.win() || model.lost() == 0) view.findNavController()
                    .navigate(R.id.action_gameFragment2_to_resultFragment2)
            } else {
                //Si no se introduce ningún texto mostramos un aviso
            ...
            model.targetWordHidden.observe(
                viewLifecycleOwner,
                Observer {
                    binding.textSecretWord.text = model.targetWordHidden.value
                })

            model.lives.observe(viewLifecycleOwner, Observer{
                binding.textLives.text= "Te quedan ${model.lives.value} vidas"
            })
        }
        return view
    }
